...
import stockfishh.libchess.Move;
import stockfishh.libchess.MoveGenerator;
import stockfishh.libchess.*;

import stockfishh.libchess.MoveList;
import stockfishh.libchess.GameContext;
...
public class Board implements Cloneable, BoardEvent {
	...
	private static Square findEnPassantTarget(Square sq, Side side) {
		...
		if (!Square.NONE.equals(sq)) {
			ep = Side.WHITE.equals(side) ? Square.encode(Rank.RANK_4, sq.getFile())
	...
	public static boolean isPromoRank(Side side, Move move) {
		if (side.equals(Side.WHITE) && move.getTo().getRank().equals(Rank.RANK_8)) {
			return true;
		} else if (side.equals(Side.BLACK) && move.getTo().getRank().equals(Rank.RANK_1)) {
			return true;
		}
	...
	public boolean doMove(final Move move, boolean fullValidation) {
		...
		if (PieceType.PAWN.equals(movingPiece.getPieceType())) {
			if (Math.abs(move.getTo().getRank().ordinal() - move.getFrom().getRank().ordinal()) == 2) {
				Piece otherPawn = Side.WHITE.equals(side) ? Piece.BLACK_PAWN : Piece.WHITE_PAWN;
	...
	/**
	 * Load an specific chess position using FEN notation ex.:
	...
	public long squareAttackedByPieceType(Square square, Side side, PieceType type) {
		...
		case BISHOP:
			...
			break;
		case ROOK:
			result = Bitboard.getRookAttacks(occ, square) & getBitboard(Piece.make(side, PieceType.ROOK));
			break;
		case QUEEN:
	...
	public boolean isMoveLegal(Move move, boolean fullValidation) {
		...
		if (bishopAndQueens != 0L && (Bitboard.getBishopAttacks(allPieces, kingSq) & bishopAndQueens) != 0L) {
			return false;
		}
		...
		if (rookAndQueens != 0L && (Bitboard.getRookAttacks(allPieces, kingSq) & rookAndQueens) != 0L) {
			return false;
		}
	...
	public boolean isAttackedBy(Move move) {
		...
		case PAWN:
			if (!move.getFrom().getFile().equals(move.getTo().getFile())) {
				attacks = Bitboard.getPawnCaptures(side, move.getFrom(), getBitboard(), getEnPassantTarget());
			} else {
	...
	public boolean isInsufficientMaterial() {
		...
		if (pawns == 0L) {
			if ((getBitboard(Piece.WHITE_QUEEN) + getBitboard(Piece.BLACK_QUEEN) + getBitboard(Piece.WHITE_ROOK)
				...
				if (count == 4) {
					if (Long.bitCount(getBitboard(Side.WHITE)) > 1 && Long.bitCount(getBitboard(Side.BLACK)) > 1) {
						result = true;
	...
	/**
	 * Get the unique position ID for the board state, which is the actual FEN
	 * representation of the board without counters
	 *
	 * @return
	 */
