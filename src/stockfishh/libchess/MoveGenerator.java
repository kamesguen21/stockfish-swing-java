...
public class MoveGenerator {
		...
		if (pieces != 0L) {
			...
			for (Square sqSource : source) {
				List<Square> target = Bitboard.bbToSquareList(
						Bitboard.getPawnCaptures(side, sqSource, board.getBitboard(), board.getEnPassantTarget())
	...
	public static void generatePawnMoves(Board board, MoveList moves) {
		...
		if (pieces != 0L) {
			List<Square> source = Bitboard.bbToSquareList(pieces);
			for (Square sqSource : source) {
				List<Square> target = Bitboard
						.bbToSquareList(Bitboard.getPawnMoves(side, sqSource, board.getBitboard()));
				for (Square sqTarget : target) {
					if (Side.WHITE.equals(side) && Rank.RANK_8.equals(sqTarget.getRank())) {
						moves.add(new Move(sqSource, sqTarget, Piece.WHITE_QUEEN));
						moves.add(new Move(sqSource, sqTarget, Piece.WHITE_ROOK));
	...
	public static void generateKnightMoves(Board board, MoveList moves) {
		...
		if (pieces != 0L) {
			...
			for (Square sqSource : source) {
				List<Square> target = Bitboard
						.bbToSquareList(Bitboard.getKnightAttacks(sqSource, ~board.getBitboard(side)));
				for (Square sqTarget : target) {
					moves.add(new Move(sqSource, sqTarget, Piece.NONE));
				}
			}
	...
	public static void generateBishopMoves(Board board, MoveList moves) {
		...
		if (pieces != 0L) {
			...
			for (Square sqSource : source) {
				List<Square> target = Bitboard.bbToSquareList(
						Bitboard.getBishopAttacks(board.getBitboard(), sqSource) & ~board.getBitboard(side));
				for (Square sqTarget : target) {
					moves.add(new Move(sqSource, sqTarget, Piece.NONE));
				}
			}
	...
	public static void generateRookMoves(Board board, MoveList moves) {
		...
		if (pieces != 0L) {
			...
			for (Square sqSource : source) {
				List<Square> target = Bitboard.bbToSquareList(
						Bitboard.getRookAttacks(board.getBitboard(), sqSource) & ~board.getBitboard(side));
				for (Square sqTarget : target) {
					moves.add(new Move(sqSource, sqTarget, Piece.NONE));
				}
			}
	...
	public static void generateQueenMoves(Board board, MoveList moves) {
		...
		if (pieces != 0L) {
			...
			for (Square sqSource : source) {
				List<Square> target = Bitboard.bbToSquareList(
						Bitboard.getQueenAttacks(board.getBitboard(), sqSource) & ~board.getBitboard(side));
				for (Square sqTarget : target) {
					moves.add(new Move(sqSource, sqTarget, Piece.NONE));
				}
			}
	...
	public static void generateKingMoves(Board board, MoveList moves) {
		...
		if (pieces != 0L) {
			...
			for (Square sqSource : source) {
				List<Square> target = Bitboard
						.bbToSquareList(Bitboard.getKingAttacks(sqSource, ~board.getBitboard(side)));
				for (Square sqTarget : target) {
					moves.add(new Move(sqSource, sqTarget, Piece.NONE));
				}
			}
