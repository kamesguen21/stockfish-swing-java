...
import stockfishh.libchess.StringUtil;
...
public class MoveList extends LinkedList<Move> implements List<Move> {
	...
	private static EnumMap<Piece, String> sanNotation = new EnumMap<Piece, String>(Piece.class);

	private static EnumMap<Piece, String> fanNotation = new EnumMap<Piece, String>(Piece.class);

	private static Map<String, PieceType> sanNotationR = new HashMap<String, PieceType>(7);
	...
	static {
		...
		fanNotation.put(Piece.WHITE_PAWN, "â™™");
		fanNotation.put(Piece.BLACK_PAWN, "â™Ÿ");
		fanNotation.put(Piece.WHITE_KNIGHT, "â™˜");
		fanNotation.put(Piece.BLACK_KNIGHT, "â™ž");
		fanNotation.put(Piece.WHITE_BISHOP, "â™—");
		fanNotation.put(Piece.BLACK_BISHOP, "â™�");
		fanNotation.put(Piece.WHITE_ROOK, "â™–");
		fanNotation.put(Piece.BLACK_ROOK, "â™œ");
		fanNotation.put(Piece.WHITE_QUEEN, "â™•");
		fanNotation.put(Piece.BLACK_QUEEN, "â™›");
		fanNotation.put(Piece.WHITE_KING, "â™”");
		fanNotation.put(Piece.BLACK_KING, "â™š");
			...
			if (Math.abs(delta) >= 2) { // is castle
				if (!board.doMove(move, true)) {
					throw new MoveConversionException(
							"Invalid move [" + move.toString() + "] for current setup: " + board.getFen());
				}
				if (delta > 0) {
					return "O-O";
				} else {
	...
	/**
	 * Get the FEN representation of the movelist applied into a standard board at
	 * the start position
	 *
	 * @param atMoveIndex
	 * @return
